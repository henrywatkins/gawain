...
class LaxFriedrichsFluxer(FluxCalculator):
    ...
    def _specific_fluxes(self, u):
       ...
       self.x_minus_flux = 0.5*((self.x_minus_flux+mid_flux_x)-dx*(u1-u2)/dt)
       ...
       self.y_minus_flux = 0.5*((self.y_minus_flux+mid_flux_y)-dy*(u1-u2)/dt)
...
class LaxWendroffFluxer(FluxCalculator):
    ...
    def _specific_fluxes(self, u):
       ...
       u2 = uplus.centroid()
       intermediate_plus_x = 0.5*((u1+u2)-dt*(self.x_plus_flux-mid_flux_x)/dx)      

       u2 = uminus.centroid()
       intermediate_minus_x = 0.5*((u1+u2)-dt*(mid_flux_x-self.x_minus_flux)/dx)
       ...
       u2 = uplus.centroid()
       intermediate_plus_y = 0.5*((u1+u2)-dt*(self.y_plus_flux-mid_flux_y)/dy)

       u2 = uminus.centroid()
       intermediate_minus_y = 0.5*((u1+u2)-dt*(mid_flux_y-self.y_minus_flux)/dy)
